 ...
 * {@link ObjectId}), item ID ({@code long} or {@link ObjectId}), preference
 * value (optional) and timestamps ("created_at", "deleted_at").
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * {@code "_id" : ObjectId("4d7627bf6c7d47ade9fc7780"), "user_id" :
 * ObjectId("4c2209fef3924d31102bd84b"), "item_id" :
 * ObjectId(4c2209fef3924d31202bd853), "preference" : 0.5, "created_at" :
 * "Tue Mar 23 2010 20:48:43 GMT-0400 (EDT)" } }
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The user IDs and item IDs are assumed to be parseable as {@code long}s or
 * {@link ObjectId}s. In case of {@link ObjectId}s, the model creates a
 * {@code Map<ObjectId>}, {@code long}> (collection "mongo_data_model_map")
 * inside the MongoDB database. This conversion is needed since Mahout uses the
 * long datatype to feed the recommender, and MongoDB uses 12 bytes to create
 * its identifiers.
 * </p>
 * 
 * <p>
 * The timestamps ("created_at", "deleted_at"), if present, are assumed to be
 * parseable as a {@code long} or {@link Date}. To express timestamps as
 * {@link Date}s, a {@link DateFormat} must be provided in the class
 * constructor. The default Date format is
 * {@code "EE MMM dd yyyy HH:mm:ss 'GMT'Z (zzz)"}. If this parameter is set to
 * null, timestamps are assumed to be parseable as {@code long}s.
 * </p>
 * 
 * <p>
 * It is also acceptable for the documents to contain additional fields. Those
 * fields will be ignored.
 * </p>
 * 
 * <p>
 * This class will reload data from the MondoDB database when
 * {@link #refresh(Collection)} is called. MongoDBDataModelExtended keeps the
 * timestamp of the last update. This variable and the fields "created_at" and
 * "deleted_at" help the model to determine if the triple (user, item,
 * preference) must be added or deleted.
 * </p>
 */
public final class MongoDBDataModelExtended implements DataModel {

	/**
	 * 
	 */
	private static final long serialVersionUID = 4875415185440797703L;

	private static final Logger log = LoggerFactory
			.getLogger(MongoDBDataModelExtended.class);
	...
	private static final DateFormat DEFAULT_DATE_FORMAT = new SimpleDateFormat(
			"EE MMM dd yyyy HH:mm:ss 'GMT'Z (zzz)", Locale.ENGLISH);
	...
	/**
	 * Creates a new MongoDBDataModelExtended
	 */
	public MongoDBDataModelExtended() throws UnknownHostException {
		this.reloadLock = new ReentrantLock();
		buildModel();
	}

	/**
	 * Creates a new MongoDBDataModelExtended with MongoDB basic configuration
	 ...
	 * @param manage
	 *            If true, the model adds and removes users and items from
	 *            MongoDB database when the model is refreshed.
	 * @param finalRemove
	 *            If true, the model removes the user/item completely from the
	 *            MongoDB database. If false, the model adds the "deleted_at"
	 ...
	 */
	public MongoDBDataModelExtended(String host, int port, String database,
	...
	/**
	 * Creates a new MongoDBDataModelExtended with MongoDB advanced
	 ...
	 * @throws UnknownHostException
	 *             if the database host cannot be resolved
	 * @see #MongoDBDataModelExtended(String, int, String, String, boolean,
	 *      boolean, DateFormat)
	 */
	public MongoDBDataModelExtended(String host, int port, String database,
			String collection, boolean manage, boolean finalRemove,
			DateFormat format, String userIDField, String itemIDField,
			String preferenceField, String mappingCollection)
			throws UnknownHostException {
		mongoHost = host;
		mongoPort = port;
		mongoDB = database;
   ...
   **/
	public MongoDBDataModelExtended(String host, int port, String database,
			String collection, boolean manage, boolean finalRemove,
			DateFormat format, String userIDField, String itemIDField, String company_id)
			throws UnknownHostException {
		mongoHost = host;
		mongoPort = port;
		mongoDB = database;
	...
	public MongoDBDataModelExtended(String host, int port, String database,
			String collection, boolean manage, boolean finalRemove,
			DateFormat format, String user, String password)
			throws UnknownHostException {
		mongoHost = host;
		mongoPort = port;
		mongoDB = database;
	 ...
	 *             if the database host cannot be resolved
	 * @see #MongoDBDataModelExtended(String, int, String, String, boolean,
	 *      boolean, DateFormat, String, String)
	 */
	public MongoDBDataModelExtended(String host, int port, String database,
			String collection, boolean manage, boolean finalRemove,
			DateFormat format, String user, String password,
	 ...
	 *            deleted
	 * @param add
	 *            If true, this flag indicates that the pairs (user, item) must
	 *            be added to the model. If false, it indicates deletion.
	 ...
	 * general contract is that any should always leave itself in a consistent,
	 * operational state, and that the refresh atomically updates internal state
	 * from old to new.
	 * </p>
	 * 
	 * @param alreadyRefreshed
	 *            s that are known to have already been refreshed as a result of
	 *            an initial call to a method on some object. This ensures that
	 *            objects in a refresh dependency graph aren't refreshed twice
	 *            needlessly.
	...
	/**
	 * <p>
	 * Translates the MongoDB identifier to Mahout/MongoDBDataModelExtended's
	 * internal identifier, if required.
	 * </p>
	 * <p>
	 ...
	 * @see <a href="http://www.mongodb.org/display/DOCS/Object%20IDs"> Mongo
	 *      Object IDs</a>
	 */
	public String fromIdToLong(String id, boolean isUser) {
		DBObject objectIdLong = collectionMap.findOne(new BasicDBObject(
				"element_id", id));
	...
	/**
	 * <p>
	 * Translates the Mahout/MongoDBDataModelExtended's internal identifier to
	 * MongoDB identifier, if required.
	 * </p>
	 * <p>
	...
	private void buildModel() throws UnknownHostException {
		...
		if (!mongoAuth
			...
			while (cursor.hasNext()) {
				Map<String, Object> user = (Map<String, Object>) cursor.next()
						.toMap();
				if (!user.containsKey("deleted_at")) {
					long userID = Long.parseLong(fromIdToLong(
							getID(user.get(mongoUserID), true), true));
					long itemID = Long.parseLong(fromIdToLong(
							getID(user.get(mongoItemID), false), false));
					float ratingValue = getPreference(user.get(mongoPreference));
					Collection<Preference> userPrefs = userIDPrefMap
							.get(userID);
					if (userPrefs == null) {
						userPrefs = Lists.newArrayListWithCapacity(2);
						userIDPrefMap.put(userID, userPrefs);
					}
					userPrefs.add(new GenericPreference(userID, itemID,
							ratingValue));
					if (user.containsKey("created_at")
							&& mongoTimestamp.compareTo(getDate(user
									.get("created_at"))) < 0) {
		...
		delegate = new GenericDataModel(GenericDataModel.toDataMap(
				userIDPrefMap, true));
	}

	private void buildModelExtended(String company_id)
			throws UnknownHostException {
		userIsObject = false;
		itemIsObject = false;
		idCounter = 0;
...
//			System.out.println(cursor.length());
	...
	private void removeMongoUserItem(String userID, String itemID) {
		...
		if (isUserItemInDB(userId, itemId)) {
			...
			if (mongoFinalRemove) {
				...
				update.put("$set", new BasicDBObject("deleted_at",
						mongoTimestamp));
	...
	private void addMongoUserItem(String userID, String itemID,
		...
		if (!isUserItemInDB(userId, itemId)) {
			...
			user.put(mongoPreference, preferenceIsString ? preferenceValue
					: Double.parseDouble(preferenceValue));
		...
		for (List<String> item : items) {
			...
			if (prefs != null) {
				...
				if (exists) {
					...
					log.info("Removing userID: {} itemID: {}", userID, itemID);
					if (mongoManage) {
						removeMongoUserItem(Long.toString(userID),
	...
	private void checkData(String userID, Iterable<List<String>> items,
			boolean add) throws NoSuchUserException, NoSuchItemException {
	...
	public String toString() {
		return "MongoDBDataModelExtended";
	}
